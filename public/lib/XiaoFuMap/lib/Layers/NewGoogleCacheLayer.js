...
(function ($, xf) {
    // ArcGIS缓存图层
    xf.NewGoogleCacheLayer = function (url) {
        xf.ImageLayer.apply(this, [url]);
        /*******************************************属性*********************************/
        /*公有*/
        //类型
        this.type = "XiaoFu.NewGoogleCacheLayer";
        //服务地址
        this.serverUrl = "";
        //图片格式
        this.picFormat = "png";
        ...
        this.update = function (extent) {
            ...
            for (var i = 0; i < numTileCols; i++) {
                ...
                for (var k = 0; k < numTileRows; k++) {
                    ...
                    if (x < 0 || y < 0 || x > Math.pow(2, z) - 1 || y > Math.pow(2, z) - 1 || $("#" + container).find("#" + id).length==1) {
