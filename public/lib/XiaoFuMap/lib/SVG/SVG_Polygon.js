...
//p_name（名称）
//p_array（点集合）
XiaoFu.SVG_Polygon = function (p_id, p_name, p_geometry) {
    ...
    this.addChild = function (p_svg, p_strPoints, p_id, p_j) {
        ...
        m_rect.setAttribute("d", p_strPoints);   //点集合
        m_rect.setAttribute("fill-rule", _self.fillRule);   //定义如何绘制元素(nonzero|evenodd)
        m_rect.setAttribute("fill", _self.fill);   //填充颜色(rgb 值、颜色名或者十六进制值)
        m_rect.setAttribute("fill-opacity", _self.fillOpacity);   //填充颜色透明度(合法的范围是：0 - 1)
